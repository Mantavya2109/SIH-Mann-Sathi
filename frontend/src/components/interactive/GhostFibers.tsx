...
/**
 * GhostFibers — animated WebGL "fibre" background (ogl).
 * Typed port of the supplied GhostFibers.jsx; behaviour and shader unchanged.
 * Pauses automatically when off-screen, when the tab is hidden, and when the
 * OS has prefers-reduced-motion on (renders one still frame instead).
 */
...
const contexts = new WeakMap<HTMLDivElement, Context>();
...
const GhostFibers = ({
  ...
  useEffect(() => {
    ...
    lineColor, glowColor, speed, scale, rotation, rotationSpeed, layers, waveAmplitude, waveFrequency,
    waveSpeed, layerSpeed, twist, twistFrequency, twistSpeed, lineFrequency, lineSpacing, lineSharpness,
    glowFalloff, glowIntensity, brightness, blueBoost, vignette, grain, lightMode, fps, paused, dpr,
  ]);
