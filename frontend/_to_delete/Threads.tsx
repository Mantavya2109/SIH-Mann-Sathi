...
/**
 * Threads — flowing Perlin-noise line field (ogl / WebGL), TypeScript port.
 *
 * Differences from the original snippet:
 *  - mouse is tracked on `window` (relative to the container), so it still
 *    reacts when other content (e.g. chat messages) sits on top of it;
 *  - with prefers-reduced-motion it renders one still frame instead of animating.
 */
...
export default function Threads({
  ...
  // Latest props live in a ref so changes update uniforms without rebuilding WebGL.
  ...
  useEffect(() => {
    ...
    // Cap internal resolution — the shader is per-pixel heavy.
    ...
    // Tracked on window so content layered above the canvas doesn't block it.
    function handleMouseMove(e: MouseEvent) {
      const rect = container!.getBoundingClientRect();
      const inside = e.clientX >= rect.left && e.clientX <= rect.right && e.clientY >= rect.top && e.clientY <= rect.bottom;
      targetMouse = inside
        ? [(e.clientX - rect.left) / rect.width, 1.0 - (e.clientY - rect.top) / rect.height]
        : [0.5, 0.5];
    }
